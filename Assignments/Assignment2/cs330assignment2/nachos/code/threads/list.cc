...
void* List::UNIX_next_thread(){
    NachOSThread* current;
    NachOSThread* temp;
    int pri = 1000;//any number greater than max priority
    ListElement* current_element = NULL;
    ListElement* prev_element = NULL;
    ListElement* prev_element_2 = NULL;
    for(ListElement *i = first; i != NULL; i = i->next){
        temp = (NachOSThread*) i->item;
        if(temp->priority < pri){
            current = temp;
            pri = current->priority;
            current_element = i;
            prev_element_2 = prev_element;
        }
        prev_element = i;
    }

    if(current_element == first) first = current_element->next;
    else if(current_element == last) {last = prev_element_2; last->next = NULL;}
    else prev_element_2->next = current_element->next;
